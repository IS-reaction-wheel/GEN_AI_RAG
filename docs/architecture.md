...
| spaCy（日本語モデル） | 日本語テキストの形態素解析（BM25 用トークナイズ）、spaCy のバイト制限対策のためのブロック分割 | BM25 キーワード検索において日本語の適切なトークン化（名詞・動詞・形容詞の抽出、助詞・記号の除外）が必要。形態素解析ベースのチャンク分割にも使用 |
...
| **構造保持チャンク分割** | チャンク分割時に文書構造の境界を無視すると検索精度が低下する | Markdown の見出し・段落構造を活用して意味的な区切りでチャンク分割を行う。短い粒度（デフォルト 500 文字）で分割し、2段階検索戦略（広域収集 + Reranker 精査）と組み合わせる |
| **テキストクリーニング** | OCR 由来のノイズ（1文字行の連続・過剰な空行等）が検索精度を低下させる | 前処理パイプラインでクリーニング（1文字行ブロックの除去、連続空行の圧縮等）を実施してから Embedding・格納を行う |
| **日本語トークナイズ** | BM25 キーワード検索では日本語の適切な分かち書きが必要 | spaCy の日本語モデルによる形態素解析を用い、名詞・動詞・形容詞を抽出してトークナイズする（助詞・記号は除外） |
